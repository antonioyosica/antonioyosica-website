...
import { withLeadingSlash, withoutTrailingSlash, withBase, joinURL, withTrailingSlash, withoutBase, parseURL } from 'ufo';
...
import { isTest, provider, isWindows } from 'std-env';
        ...
        if (nitro.options.compressPublicAssets && assetData.length > 1024 && !assetId.endsWith(".map")) {
          for (const encoding of ["gzip", "br"]) {
            ...
            const compressedBuff = await new Promise((resolve2, reject) => {
              zlib[encoding === "gzip" ? "gzip" : "brotliCompress"](
                assetData,
                (error, result) => error ? reject(error) : resolve2(result)
              );
            });
            await promises.writeFile(compressedPath, compressedBuff);
